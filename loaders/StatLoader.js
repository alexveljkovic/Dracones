...
function parseStatFormula(statObj) {
  ...
  switch (statType) {
    ...
    case 'constant':
      statName = statObj.value;
      if (availableStats[statName] == null) {
        const newStat = new ConstantStat({ name: statName, defaultValue: statObj.value });
    ...
    case 'op':
      return new StatOperator(
        { opcode: statObj.op, dependencies: statObj.args.map((arg) => parseStatFormula(arg)) },
      );
    default:
  ...
  switch (section) {
    case 'Stats':
      group.items.forEach((stat) => {
        ...
        const statData = { name: statName, modifiers: utils.constants.DND_MODIFIERS };
        ...
        if (statProperties.length === 0) {
          ...
          if (propertiesObject.formula == null) {
            ...
            availableStats[statName] = new Stat({ ...statData, dependencies });
          }
        }
      });
      break;
    default: throw Error(`Invalid section type: ${section}`);
  }
});

module.exports = Object.keys(availableStats).map((key) => availableStats[key]);
